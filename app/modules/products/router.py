...
@router.post("/", response_model=ProductRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(RoleGuard(UserRole.OWNER, UserRole.ADMIN))])
...
@router.get("/", response_model=list[ProductRead], dependencies=[Depends(RoleGuard(UserRole.OWNER, UserRole.ADMIN, UserRole.STAFF))])
...
@router.patch("/{product_id}", response_model=ProductRead, dependencies=[Depends(RoleGuard(UserRole.OWNER, UserRole.ADMIN))])
...
@router.delete("/{product_id}", response_model=ProductRead, dependencies=[Depends(RoleGuard(UserRole.OWNER, UserRole.ADMIN))])
